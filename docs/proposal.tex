...
\texttt{.deb} installer as required. The API URL and \texttt{X-API-Key} are read
from environment configuration rather than entered in a startup dialog. The
final application uses six tabs: \emph{Players}, \emph{Standings},
\emph{Rating History}, \emph{Record Game}, \emph{Register Player} and
\emph{Create Tournament}. tkinter is chosen because it ships with the Python
standard library and packages cleanly into a Debian archive.

\begin{figure}[ht]
\centering
\begin{minipage}[t]{0.47\linewidth}\centering
  \includegraphics[width=\linewidth]{assets/figure_2.png}
  \captionof{figure}{Early connection-dialog sketch from the planning phase; final configuration is environment-based.}
...
planning sketches. During implementation the interface was simplified into a
single six-tab tkinter window. The \emph{Players} tab lists players and creates
new ones; \emph{Create Tournament} creates a tournament and displays the
new round IDs; \emph{Register Player} connects players to tournaments;
\emph{Record Game} records results; \emph{Standings} displays the calculated
ranking; and \emph{Rating History} displays all Elo-history rows or filters
them by player. Successful writes refresh the relevant read views so newly
created players and tournaments become selectable without restarting the GUI.

% ============================================================
\section{Operation with Docker Compose}
% ============================================================

The active \texttt{docker-compose.yml} service is \texttt{postgres}, using the
official \texttt{postgres:16} image with persistent storage and the configured
database credentials. The FastAPI application is started locally with Uvicorn
from the Python environment and connects to PostgreSQL using values from the
\texttt{.env} configuration. The repository includes \texttt{scripts/run-local.sh}
to load the environment, start PostgreSQL, start the API and then launch the
tkinter frontend. The frontend can also be built as a Debian package and
installed separately. This separation keeps the database reproducible through
Docker while allowing the Python API and desktop GUI to run directly on the
host system.
...
health endpoint, and a 6-tab tkinter frontend. Automatic Swiss pairing,
player-opening analytics, user accounts, edit/delete workflows and tournament
closing are explicitly outside the final scope.
...
4--5  & tkinter frontend with six final tabs, local launcher and \texttt{.deb} packaging & 10 \\
6     & \texttt{pytest} and database smoke tests, LaTeX documentation with CI, and 8--10 min demonstration video & 10 \\
\bottomrule
\end{tabularx}
